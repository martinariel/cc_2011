...
class GlobalFunctions
{
    ...
    {
        if ( !self::needsFotoPrincipal( $media ) ) return false;
        $image  = false;
        
        // obtener img handler
        $img    = self::redimensionarImagen( self::webDir . '/' . $media->getArchivo() );
        ...
        if ( $image )
        {
            ...
            $fotoPrincipal->setCreatedAt( $media->getCreatedAt() );
    ...
    public static function redimensionarImagen ( $imgPath , $resize = false )
    {
        ...
        if ( $oldWidth == false || $oldHeight == false ) return false;
          ...
          // nuevas dimensiones 
          // FIXME quitar este hardcore horrible
          $dim = array( 90, 120 );
          $i   = $oldWidth >= $oldHeight ? 1 : 0;
          list( $newWidth, $newHeight ) = array( $dim[$i] , $dim[+!$i]); 
    ...
    public static function needsFotoPrincipal ( $media )
    {
        if ( !is_object( $media ) )     
            return false;
        
        if ( ( $persona = $media->getPersona() ) == null || !is_object( $persona ) )  
            return false;
    
        if ( ( $fp = MediaPeer::retrieveFotoPrincipal( $persona ) ) == null ) 
        {
            return true;
        }
        if ( $fp->getCreatedAt('U') >= $media->getCreatedAt('U') )
        {
            return false;
        }
        else
